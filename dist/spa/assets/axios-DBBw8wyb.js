import { j as je } from './index-DAawzX9R.js';
function me(e, t) {
  return function () {
...
const mt = (e) => {
  ...
  yt = A('AsyncFunction'),
  Et = (e) => e && (q(e) || R(e)) && R(e.then) && R(e.catch),
  a = {
    isArray: C,
    isArrayBuffer: ye,
    isBuffer: Ie,
...
function M(e, t, n) {
  ...
  function l(f, d, b) {
    ...
    if (f && !b && typeof f == 'object') {
      if (a.endsWith(d, '{}'))
        ...
        (a.isArray(f) && wt(f)) ||
        ((a.isFileList(f) || a.endsWith(d, '[]')) && (w = a.toArray(f)))
      )
        return (
          (d = ge(d)),
          w.forEach(function (x, ke) {
  ...
  function S(f, d) {
    if (!a.isUndefined(f)) {
      ...
      u.push(f),
        a.forEach(f, function (w, g) {
          (!(a.isUndefined(w) || w === null) &&
            s.call(t, w, a.isString(g) ? g.trim() : g, d, E)) === !0 &&
            S(w, d ? d.concat(g) : [g]);
        }),
        u.pop();
    }
  ...
  De = {
    getAdapter: (e) => {
      ...
      for (let o = 0; o < t; o++) {
        ...
        if (
          ((r = n),
          !Gt(n) && ((r = X[(i = String(n)).toLowerCase()]), r === void 0))
        )
...
const v = { assertOptions: Xt, validators: ne },
  N = v.validators;
class j {
  constructor(t) {
    (this.defaults = t),
...
y.HttpStatusCode = Yt;
y.default = y;
const en = y.create({ baseURL: 'https://api.example.com' }),
  nn = je(({ app: e }) => {
    (e.config.globalProperties.$axios = y),
      (e.config.globalProperties.$api = en);
  });
export { en as api, nn as default };
